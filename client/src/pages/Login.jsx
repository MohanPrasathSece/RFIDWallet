    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-green-50 to-white">
      ...
      <header className="sticky top-0 z-10 backdrop-blur border-b border-white/60 bg-white/80">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="text-xs text-green-800/70 hidden md:block">Secure role-based access — Admin and Student</div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto min-h-[calc(100vh-56px)] grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="relative flex items-center justify-center p-8 md:p-12 overflow-hidden">
          ...
          <div className="absolute bottom-10 -right-10 w-56 h-56 rounded-full bg-green-400/20 blur-2xl" />
        ...
        <div className="relative flex items-center justify-center p-8 md:p-12 overflow-hidden">
          ...
          {/* decorative circles */}
          <div className="absolute -top-12 right-6 w-48 h-48 rounded-full bg-emerald-400/20 blur-2xl" />
          <div className="absolute bottom-6 -left-10 w-56 h-56 rounded-full bg-green-400/20 blur-2xl" />

          <div className="w-full max-w-md backdrop-blur-sm bg-white/85 border border-white/60 shadow-2xl rounded-2xl p-6 transition hover:shadow-[0_20px_50px_-20px_rgba(16,185,129,0.45)]">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-emerald-600 flex items-center justify-center text-white text-lg">🎓</div>
              <div>
                <h2 className="text-2xl font-semibold text-emerald-900">For Students</h2>
                <p className="text-xs text-emerald-800/80">Login to view wallet, transactions and library.</p>
              </div>
            </div>
            ...
            <form onSubmit={onStudentSubmit} className="mt-4">
              <label className="text-xs font-medium text-emerald-900/80">Roll Number</label>
              <input className="input mt-1 focus:ring-2 focus:ring-emerald-400" placeholder="e.g., 20CS123" value={rollNo} onChange={e=>setRollNo(e.target.value)} />
              <label className="text-xs font-medium text-emerald-900/80 mt-3 block">Password</label>
              <div className="relative mt-1">
                <input className="input pr-10 focus:ring-2 focus:ring-emerald-400" type={studentShowPw ? 'text' : 'password'} placeholder="••••••••" value={studentPassword} onChange={e=>setStudentPassword(e.target.value)} />
                <button type="button" onClick={()=>setStudentShowPw(v=>!v)} className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-emerald-700/80 hover:text-emerald-900">
                  {studentShowPw ? 'Hide' : 'Show'}
                </button>
              </div>
              <div className="mt-3 flex items-center justify-between">
                <p className="text-[11px] text-emerald-900/70">Accounts are created by Admin. No public signup.</p>
                <span className="text-[11px] text-emerald-900/50">Need help? Contact Admin</span>
              </div>
              <button className="w-full mt-4 py-2.5 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white font-medium transition disabled:opacity-60 active:scale-[.99]" disabled={studentLoading}>
